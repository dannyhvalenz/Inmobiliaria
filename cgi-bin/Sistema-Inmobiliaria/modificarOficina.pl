...
my $sth = $dbh->prepare("UPDATE Oficina SET metrosCuadrados=?, numBanos=?, edadOficina=?, areaAbierta=?, 
                    metrosAreaAbierta=?, privados=?, numPrivados=?, estacionamiento=?, numCoches=?, recepcion=? WHERE idInmueble=?");
$sth->execute($metrosCuadrados, $numBanos, $edadOficina, $areaAbierta,
                    $metrosAreaAbierta, $privados, $numPrivados, $estacionamiento, $numCoches, $recepcion, $idInmueble) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Oficina actualizada');
    $writer->dataElement('contenido', 'Se ha actualizado la oficina');
} else {
    $writer->dataElement('titulo', 'Error de conexion');
    $writer->dataElement('contenido', 'No se ha podido registrar la oficina');
}
