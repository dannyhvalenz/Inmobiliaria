...
my $sth = $dbh->prepare("INSERT into Casa (idInmueble, metrosCuadrados, metrosConstruccion, numPisos, numRecamaras, numBanos, edadCasa, tipoCasa, tipoCocina,
                                            ...
                                            cuartoServicio, sala, comedor, jardin, areaLavado, bodega) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$sth->execute($idInmueble, $metrosCuadrados, $metrosConstruccion, $numPisos, $numRecamaras, $numBanos, $edadCasa, $tipoCasa, $tipoCocina,
                $cochera, $numCoches, $cocheraTechada, $fraccionamiento, $fraccionamientoVigilancia, $fraccionamientoAreaVerde, $fraccionamientoCasaClub,
                $cuartoServicio, $sala, $comedor, $jardin, $areaLavado, $bodega) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nueva casa');
    $writer->dataElement('contenido', 'Se ha registrado la casa');
} else {
