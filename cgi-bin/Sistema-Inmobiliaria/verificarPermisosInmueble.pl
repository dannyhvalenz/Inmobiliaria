...
my $sth = $dbh->prepare("SELECT idAsesor from Propietario WHERE idPropietario=?");
...
if ($sth->rows gt 0){
    ...
    $writer->dataElement('titulo', "Asesor Recuperado");
    $writer->dataElement('contenido', $row->{idAsesor});
} else {
