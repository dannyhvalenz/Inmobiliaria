...
my $sth = $dbh->prepare("INSERT into Terreno (idInmueble, metrosCuadrados, usoSuelo, poligonal, pendiente, esquina) values (?,?,?,?,?,?)");
$sth->execute($idInmueble, $metrosCuadrados, $usoSuelo, $poligonal, $pendiente, $esquina) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo terreno');
    $writer->dataElement('contenido', 'Se ha registrado el terreno');
} else {
