...
my $sth = $dbh->prepare("INSERT into Edificio (idInmueble, metrosConstruccion, numPisos, numBanos, edadEdificio) values (?,?,?,?,?)");
$sth->execute($idInmueble, $metrosConstruccion, $numPisos, $numBanos, $edadEdificio) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo edificio');
    $writer->dataElement('contenido', 'Se ha registrado el edificio');
} else {
