...
if ($sth->rows gt 0){
    while (my $row = $sth->fetchrow_hashref){
        ...
        $writer->dataElement('ciudad', $row->{ciudad});
        $writer->dataElement('colonia', $row->{colonia});
        $writer->dataElement('calle', $row->{calle});
        $writer->dataElement('numExt', $row->{numExt});
        $writer->dataElement('numInt', $row->{numInt});
        $writer->dataElement('precio', $row->{precio});
        $writer->dataElement('tipoInmueble', $row->{tipoInmueble});
        $writer->dataElement('tipoTransaccion', $row->{tipoTransaccion});
        $writer->dataElement('idPropietario', $row->{idPropietario});
    }
} else {
