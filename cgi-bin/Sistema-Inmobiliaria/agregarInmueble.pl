...
my $sth = $dbh->prepare("INSERT into Inmueble (idPropietario, ciudad, colonia, calle, numExt, numInt, precio, tipoTransaccion, tipoInmueble) values (?,?,?,?,?,?,?,?,?)");
$sth->execute($idPropietario, $ciudad, $colonia, $calle, $numExt, $numInt, $precio, $tipoTransaccion, $tipoInmueble) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo inmueble');
    $writer->dataElement('contenido', 'Se ha registrado el inmueble');
    $writer->dataElement('idInmueble', $last_insert_id);
} else {
