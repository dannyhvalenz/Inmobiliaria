...
if ($sth->rows gt 0){
    while (my $row = $sth->fetchrow_hashref){
        ...
        $writer->dataElement('idDepartamento', $row->{idDepartamento});
        $writer->dataElement('metrosConstruccion', $row->{metrosConstruccion});
        $writer->dataElement('numRecamaras', $row->{numRecamaras});
        $writer->dataElement('numPiso', $row->{numPiso});
        $writer->dataElement('espacioHabitable', $row->{espacioHabitable});
        $writer->dataElement('numBanos', $row->{numBanos});
        $writer->dataElement('edadDepartamento', $row->{edadDepartamento});
        $writer->dataElement('tipoDepartamento', $row->{tipoDepartamento});
        ...
        $writer->dataElement('estacionamientoTechado', $row->{estacionamientoTechado});
        $writer->dataElement('vigilancia', $row->{vigilancia});
