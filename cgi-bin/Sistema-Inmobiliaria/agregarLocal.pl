...
my $sth = $dbh->prepare("INSERT into Local (idInmueble, metrosCuadrados, dentroPlaza, numBanos, edadLocal) values (?,?,?,?,?)");
$sth->execute($idInmueble, $metrosCuadrados, $dentroPlaza, $numBanos, $edadLocal) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo local');
    $writer->dataElement('contenido', 'Se ha registrado el local');
} else {
