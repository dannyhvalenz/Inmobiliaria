...
my $usoSuelo = $query->param("usoSuelo");
my $poligonal = $query->param("poligonal");
my $pendiente = $query->param("pendiente");
my $esquina = $query->param("esquina");
# la sección de acceso a Mysql
# Documentación en:
# https://metacpan.org/pod/DBD::mysql
my $dbh = conectar();
# Realizar el query sql
my $sth = $dbh->prepare("UPDATE Terreno SET metrosCuadrados=?, usoSuelo=?, poligonal=?, pendiente=?, esquina=? WHERE idInmueble=?");
$sth->execute($metrosCuadrados, $usoSuelo, $poligonal, $pendiente, $esquina,$idInmueble) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Terreno actualizado');
    $writer->dataElement('contenido', 'Se ha actualizado el terreno');
} else {
    $writer->dataElement('titulo', 'Error de conexion');
    $writer->dataElement('contenido', 'No se ha podido actualizar el terreno');
}
