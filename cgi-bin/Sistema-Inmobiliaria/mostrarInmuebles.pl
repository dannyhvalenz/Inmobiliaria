...
if ($sth->rows gt 0){
    while (my $row = $sth->fetchrow_hashref){
        ...
        $writer->startTag('div', class => 'w3-white w3-quarter w3-padding-small', id => 'contenedor_principal', style => 'min-height: 270px;');
            $writer->startTag('div', class => 'w3-white', id => 'contenedor_propiedad',  style => 'min-height: 270px;');
                $writer->startTag('div', class => 'w3-card w3-hover-light-grey', onclick => "w3_open_actualizar(this.id)", id => "$row->{idInmueble}" , style => 'min-height: 270px;');
