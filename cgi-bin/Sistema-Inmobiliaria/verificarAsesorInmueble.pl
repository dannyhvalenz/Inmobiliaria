...
my $sth = $dbh->prepare("SELECT idPropietario from Inmueble WHERE idInmueble=?");
...
if ($sth->rows gt 0){
    ...
    $writer->dataElement('titulo', "Propietario Recuperado");
    $writer->dataElement('contenido', $row->{idPropietario});
}
else {
