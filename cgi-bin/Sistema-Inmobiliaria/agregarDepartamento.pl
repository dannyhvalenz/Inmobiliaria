...
my $sth = $dbh->prepare("INSERT into Departamento (idInmueble, metrosConstruccion, numRecamaras, numPiso, espacioHabitable, numBanos, edadDepartamento, tipoDepartamento, tipoCocina, 
                                            estacionamiento, numCoches, estacionamientoTechado, vigilancia, cuartoServicio, sala, comedor, jardin, areaLavado, bodega) 
                                            values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$sth->execute($idInmueble, $metrosConstruccion, $numRecamaras, $numPiso, $espacioHabitable, $numBanos, $edadDepartamento, $tipoDepartamento, $tipoCocina, 
                $estacionamiento, $numCoches, $estacionamientoTechado, $vigilancia, $cuartoServicio, $sala, $comedor, $jardin, $areaLavado, $bodega) or die $DBI::errstr;
...
if ($sth->rows gt 0){
    $writer->dataElement('titulo', 'Nuevo departamento');
    $writer->dataElement('contenido', 'Se ha registrado el departamento');
} else {
