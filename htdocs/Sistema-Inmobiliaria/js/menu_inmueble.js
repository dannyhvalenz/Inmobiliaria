...
function buscar() {
    ...
    if (busqueda == null || busqueda == ""){
        ...
        xhr.open("POST", "http://localhost:8888/cgi-bin/Sistema-Inmobiliaria/buscarInmueble.pl", true);
...
function w3_open_actualizar(id) {
    var path = "gestionar_Inmueble.html?idInmueble="+id+"&idAsesor="+idAsesor+"&nombreAsesor="+nombreAsesor+"&link="+linkWhenRefresh;
...
function w3_open_registro() {
    var path = "registro_Inmueble.html?idAsesor="+idAsesor+"&nombreAsesor="+nombreAsesor+"&link="+linkWhenRefresh;
    document.getElementById("divAdmin").style.display = "block";
    document.getElementById("divVisualizar").style.width = "50%";
    document.getElementById("divAdmin").style.width = "50%";
    document.getElementById("divAdmin").style.height = "100vh";
...
/* FUNCION PARA CARGAR TODOS LOS INMUEBLES EN EL DIV QUE CORRESPONDE */
function cargarInmuebles(){
    ...
    xhr.open("POST", "http://localhost:8888/cgi-bin/Sistema-Inmobiliaria/mostrarInmuebles.pl", true);
    xhr.setRequestHeader(
    'Content-type', 
    'application/x-www-form-urlencoded'
    ); 
